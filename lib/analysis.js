...
export function analyze(input) {
  ...
  if (ivHv != null) {
    factors.push({
      label: t(lang, 'factorIvHv'),
      value: ivHv.toFixed(2),
      tone: tone(ivHv < 0.9, ivHv <= 1.15),
      note:
        ivHv < 0.9
          ? t(lang, 'factorIvHvCheap')
          : ivHv <= 1.15
          ? t(lang, 'factorIvHvNeutral')
          : t(lang, 'factorIvHvRich'),
    });
  }
  if (input.ivRankPct !== '' && input.ivRankPct != null && !Number.isNaN(+input.ivRankPct)) {
    const rank = +input.ivRankPct;
    factors.push({
      label: t(lang, 'factorIvRank'),
      value: rank.toFixed(0),
      tone: tone(rank < 30, rank <= 60),
      note:
        rank < 30
          ? t(lang, 'factorIvRankLow')
          : rank <= 60
          ? t(lang, 'factorIvRankMid')
          : t(lang, 'factorIvRankHigh'),
    });
  }
  factors.push({
    label: t(lang, 'factorPop'),
    value: (pop * 100).toFixed(0) + '%',
    tone: tone(pop >= 0.5, pop >= 0.4),
    note: t(lang, 'factorPopNote', { drift: (mu * 100).toFixed(1) }),
  });
  factors.push({
    label: t(lang, 'factorBreakeven'),
    value:
      (breakevenMovePct >= 0 ? '+' : '') + (breakevenMovePct * 100).toFixed(1) + '%',
    tone: tone(reachRatio <= 1, reachRatio <= 1.5),
    note: t(lang, 'factorBreakevenNote', {
      ratio: reachRatio.toFixed(2),
      expected: (expected * 100).toFixed(1),
    }),
  });
  if (rewardRisk != null) {
    factors.push({
      label: t(lang, 'factorRewardRisk'),
      value: rewardRisk >= 0 ? rewardRisk.toFixed(2) + ' : 1' : t(lang, 'factorRewardRiskLoss'),
      tone: tone(rewardRisk >= 2, rewardRisk >= 1),
      note: t(lang, 'factorRewardRiskNote', {
        target,
        sign: profitAtTarget >= 0 ? '' : '-',
        profit: Math.abs(profitAtTarget).toFixed(0),
  ...
  if (richVol) {
    badge = t(lang, 'badgeExpensive');
    badgeTone = 'bad';
    summary = t(lang, 'badgeExpensiveSummary');
  } else if (longShot) {
    badge = t(lang, 'badgeLongShot');
    badgeTone = 'bad';
    summary = t(lang, 'badgeLongShotSummary');
  } else if (sum >= 2) {
    badge = t(lang, 'badgeReasonable');
    badgeTone = 'good';
    summary = t(lang, 'badgeReasonableSummary');
  } else if (sum >= 0) {
    badge = t(lang, 'badgeMarginal');
    badgeTone = 'ok';
    summary = t(lang, 'badgeMarginalSummary');
  } else {
    badge = t(lang, 'badgePoor');
    badgeTone = 'bad';
    summary = t(lang, 'badgePoorSummary');
  }
  // Single comparable number for ranking contracts against each other (e.g. a
  // "best deal" sort across a whole chain) — badge overrides always sort below
  // any non-overridden reading, matching the priority above.
  const score = richVol ? -10 : longShot ? -5 : sum;
  ...
  return {
    ...
    profitAtTarget,
    target,
    verdict: { badge, badgeTone, summary, factors, score },
  };
}
